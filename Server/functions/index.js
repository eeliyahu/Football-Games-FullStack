...
app.use(cors({ origin: 'https://football-games-project.web.app' }));
var urlencodedParser = bodyParser.urlencoded({ extended: true })
app.use(urlencodedParser);
app.use(bodyParser.json());

app.get('/', (req, res) => {
    res.send('<h1>Football Games Server</h1> Server is running successfully</p>');
});  
...
app.get('/getGames/', function (req, res) {
    try {
        ...
        let league = 'ALL';
        let yearParam = (year === 'Upcoming')? 'fixture=true' : `season=${year}`;
